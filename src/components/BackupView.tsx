...
import { Database, FileSpreadsheet, Download, Upload, Trash2, HelpCircle } from "lucide-react";
...
export default function BackupView({
  ...
  const exportToExcel = () => {
    ...
    const metaRows = [
      ["COLEGIO:", state.config.school],
      ["DOCENTE:", state.config.teacher],
      ["GRADO:", g.label],
      ["MATERIA:", g.useGlobalSubject ? state.config.subject : g.subject],
      ["TIEMPO:", labelPeriodDisplay],
      [] // separator row
    ];
    ...
    if (isAnual) {
      ...
      headers.push("PROMEDIO ANUAL");

      rows = activeList.map(s => {
        const studentRow = [s.name];
        let sum = 0;
      ...
      headers = ["Estudiante"];
      for (let i = 1; i <= 10; i++) headers.push(`${b[0]} ${i}`);
      headers.push(`Promedio ${b[0]}`);
      for (let i = 1; i <= 10; i++) headers.push(`${b[1]} ${i}`);
      headers.push(`Promedio ${b[1]}`, b[2], b[3], b[4], `${b[5]} (Examen)`, `${b[5]} (Rúbrica)`, `Promedio Exámenes`, "PROMEDIO FINAL");
      ...
      rows = pActiveList.map(s => {
        const studentRow: (string | number | null)[] = [s.name];
    ...
    // Write out workbook
    XLSX.writeFile(wb, `Registro_${g.label.replace(/\s+/g, "_")}_${labelPeriodInternal}.xlsx`);
  };
  ...
  const handleExcelUpload = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      ...
      if (studentHeaderRowIndex === -1) {
        alert("Estructura de Excel inválida. No se localizó la columna del encabezado 'Estudiante'.");
        return;
      }
    ...
    reader.onload = (evt) => {
      try {
        ...
        if (parsed.config && parsed.data) {
          ...
          alert("El archivo no es un respaldo de calificaciones válido.");
        }
      } catch (err) {
